...
def score(gan,iterator,next_set,batch_size,method="mse",add_noise=False,
        noise_std=0.025):
    if add_noise:
    ...
    while True:
        try:
            ...
            for i in range(int(np.floor(img_A.shape[0]/batch_size))):
                ...
                temp = img_A[items]
                if add_noise:
                    temp = img_A[items] + rng.normal(0,noise_std,
                                                           size=img_A[items].shape)
                total_loss += gan.score(temp,img_B[items],method)
                k+=1
...
def main(checkpoint_dir,
        ...
        model,dataset,experiment_id,semi_loss_weight,only_pair,add_noise):
    ...
    # begin training
    try:
        results_df = pd.read_csv("noise_results.csv")
    except:
        results_df = pd.DataFrame(columns=["test_mse","test_mae","model","noise","experiment"])
    results = {}
    results["model"] = model
    results["experiment"] = experiment_id
    for ele in [0.025,0.05,0.1,0.15,0.2,0.3,0.4]:
        test_score = score(gan,iterator_test,next_test,batch_size,add_noise=True,noise_std=ele)
        print("test_mse: {:.04f}".format(test_score))
        results["test_mse"] = test_score
        test_score = score(gan,iterator_test,next_test,batch_size,"mae",add_noise=True,noise_std=ele)
        print("test_mae: {:.04f}".format(test_score))
        results["test_mae"] = test_score
        results["noise"] = ele
        results_df = results_df.append(results,ignore_index=True)
        results_df.to_csv("noise_results.csv",index=False)
